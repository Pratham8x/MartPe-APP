...
const AddNewAddress: React.FC = () => {
  ...
  const getcityName = async (latitude, longitude) => {
    ...
    try {
      // const urlNew_Response = await axios.get(urlNew);
      const url_Response = await axios.get(url);
      setAddressInput((prev) => ({
        ...prev,
        address: {
          line1: url_Response?.data.items[0].address.label,
          line2: url_Response?.data.items[0].address.label,
        },
        city: url_Response?.data.items[0].address.city,
        state: url_Response?.data.items[0].address.state,
        pincode: url_Response?.data.items[0].address.postalCode,
        locality:
          url_Response?.data.items[0].address.street +
          ", " +
          url_Response?.data.items[0].address.district,
      }));
    ...
    <View>
      <View
        style={{
          ...
          borderWidth: 1,
          borderColor: "#d6d6d6",
          marginBottom: !isDropdownExpanded && 10,
        }}
      >
        <TouchableOpacity
